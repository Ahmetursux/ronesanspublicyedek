...
return msg.channel.send(queueembed)
  ...
  if(`${args[0]}` == `loop`){
		if(!serverQueue) return msg.channel.send('❌ | Şuan Bir Şey Oynatılmıyor');
		if(!msg.channel.send) return msg.channel.send('❌ | Lütfen Ses Kanalına Katıl');
...
var pleyembed = new RichEmbed() 
  ...
  .addField('Şarkıyı Açan', `${song.meminta}`)
  .setFooter("Botta kesilme veya herhangi bir sorun yaşıyorsanız geliştiricime mesaj atabilirsiniz!")
  .setTimestamp();
